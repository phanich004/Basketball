...
# MediaPipe has compatibility issues on ARM Macs, so we'll run without pose detection
MEDIAPIPE_AVAILABLE = False
print("Running without MediaPipe pose detection (using fixed position for demo)")

# Load shot data from JSON (handle comments)
with open('ball.json', 'r') as f:
    ...
    cleaned_content = '\n'.join(cleaned_lines)
    shot_data = json.loads(cleaned_content)

# Initialize MediaPipe Pose if available
pose = None

# Open the video files
process_video_path = 'final_ball.mov'
display_video_path = 'final_ball.mov'
...
while process_cap.isOpened() and display_cap.isOpened():
    ...
    if frame_count % process_every_n_frames == 0 and MEDIAPIPE_AVAILABLE and pose is not None:
    ...
    # If MediaPipe is not available, use center of frame as fallback
    if not MEDIAPIPE_AVAILABLE and last_head is None:
        last_head = (display_width // 2, display_height // 3)
